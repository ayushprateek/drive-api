...
class CitySerializer(serializers.ModelSerializer):
    ...
    def get_components(self, obj):
        ...
        # Extract Site Obj.
        site_obj = trip_models.Site.filter_instance({"city": obj}).first()
        if site_obj:
            component_list.append({"type": Constant.SITES,
                                   **CommonSerializer(site_obj).data})
...
class CityDetailSerializer(serializers.ModelSerializer):
    ...
    def get_components(self, obj):
        ...
        # Extract Site Obj.
        site_obj = trip_models.Site.filter_instance({"city": obj})
        if site_obj:
            data = CommonSerializer(site_obj[:3], many=True).data
            component_list.append({"type": Constant.SITES,
                                   "image_url": trip_models.Category.get_instance(
                                       {"name": Constant.SITES}).image_url,
                                   "total_count": site_obj.count(),
                                   "data": data})
