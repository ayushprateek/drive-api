...
class SendEmailOTP(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            # If validation error occurs, send a proper response with the error message
            return Response({'detail': str(ve)}, status=400)

        except Exception as e:
            # Log any other exceptions
            print("Error:", str(e))  # You can print to console or save to logs
            # traceback.print_exc()  # Print the detailed stack trace for debugging
            return Response({'detail': 'Internal server error occurred.'}, status=500)
